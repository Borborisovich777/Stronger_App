...
export type WorkoutStartingWeightIncrease = Omit<WorkoutLoadIncrease, "evidenceSessionIds">;
...
/** Recover the start-only explanation from this workout, including after a reload or import. */
export function getWorkoutStartingWeightIncreases(workout: WorkoutSession): WorkoutStartingWeightIncrease[] {
  if (workout.finishedAt !== undefined || !workout.startingWeightAdjustments?.length) return [];
  const adjustments = new Map(workout.startingWeightAdjustments.map((item) => [item.setId, item]));
  return workout.exercises.flatMap((exercise) => {
    if (resolveExerciseTracking(exercise) !== "weight-reps" ||
      resolveExerciseWeightMode(exercise) === "assistance") return [];
    const matched = exercise.sets.filter((set) => !set.dropSetOf).flatMap((set) => {
      const adjustment = adjustments.get(set.id);
      return adjustment && Number.isFinite(adjustment.previousWeightKg) && adjustment.previousWeightKg > 0 &&
        Number.isFinite(adjustment.nextWeightKg) && adjustment.nextWeightKg > adjustment.previousWeightKg
        ? [adjustment] : [];
    });
    if (!matched.length) return [];
    const incrementKg = matched[0].nextWeightKg - matched[0].previousWeightKg;
    const sets = matched.filter((item) => Math.abs(item.nextWeightKg - item.previousWeightKg - incrementKg) <= WEIGHT_TOLERANCE_KG);
    return [{ exerciseId: exercise.id, exerciseKey: exercise.exerciseKey, exerciseName: exercise.name, incrementKg, sets }];
  });
}

/** Preserve the upper goal in notes generated by older prepared templates without rewriting them. */
export function resolveProgressionRepTarget(template: RoutineExercise): number {
  if (template.progressionRepTarget !== undefined) {
    return Math.max(template.targetReps, template.progressionRepTarget);
  }
  // This exact leading format came from templateTargetLabel + createRoutineFromTemplate.
  // Arbitrary exercise notes are not a source of numeric progression settings.
  const generatedRange = template.notes?.match(/^(\d+) × (\d+)–(\d+) reps(?: \/ side)?\. Rest (\d+)s\.(?:$| )/);
  if (generatedRange) {
    const [, count, lower, upper, rest] = generatedRange.map(Number);
    if (Number.isInteger(count) && count === template.targetSets && count > 0 &&
      Number.isInteger(lower) && lower > 0 && Number.isInteger(upper) &&
      lower <= upper && upper <= MAX_REPS &&
      template.targetReps >= lower && template.targetReps <= upper &&
      Number.isInteger(rest) && rest >= 0 && rest <= 86_400) {
      return upper;
    }
  }
...
export function applyNextWorkoutProgression(
  ...
  if (!Number.isFinite(incrementKg) || incrementKg <= 0 || incrementKg > MAX_INCREMENT_KG ||
    ...
    workout.finishedAt !== undefined || workout.startingWeightAdjustments !== undefined || workout.sourceRoutineId !== routine.id ||
  ...
  for (const [identity, rows] of groups) {
    ...
    const baseline = rows.flatMap(({ template, exercise }) => exercise.sets.map((set) => ({
      ...set,
      progressionReps: resolveProgressionRepTarget(template),
    })));
  ...
  return increases.length ? {
    workout: { ...workout, exercises: updatedExercises, startingWeightAdjustments: increases.flatMap((increase) => increase.sets) },
    increases,
  } : unchanged;
}
